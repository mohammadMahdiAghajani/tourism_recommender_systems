...
"""
مدل `AttractionProfiles` اطلاعات تکمیلی و ویژگی‌های دسته‌بندی‌شدهٔ هر 
جاذبه را برای استفاده در فیلترها و موتور پیشنهاددهی نگهداری می‌کند و از طریق رابطهٔ
 یک‌به‌یک `attraction` به مدل `Attractions` متصل است؛ این فیلد هم‌زمان کلید اصلی
 جدول نیز محسوب می‌شود و با `related_name='profile'` امکان دسترسی 
 به پروفایل از طریق `attraction.profile` را فراهم می‌کند. فیلدهای مدل، مشخصاتی مانند نوع و 
 انگیزهٔ بازدید، محیط جاذبه، محدودهٔ هزینه، همراهان مناسب، 
 تحرک موردنیاز، زمان و فصل پیشنهادی، تجربهٔ فرهنگی، 
 عنصر طبیعی غالب، فعالیت گردشگری، سطح دسترسی، 
 مدت بازدید و محبوبیت را ثبت می‌کنند؛ مقدار `environment` نیز به گزینه‌های از 
 پیش تعریف‌شده محدود شده است. گزینهٔ `managed=False` نشان می‌دهد جدول Legacy با
 نام `attraction_profiles` از قبل در پایگاه داده وجود دارد 
 و Django نباید آن را از طریق migration ایجاد، 
 تغییر یا حذف کند. همچنین املای نادرست `cltural_experience` عمداً برای سازگاری
 با نام ستون موجود در ساختار اصلی پایگاه داده حفظ شده است.
"""
...
"""
مدل `UserProfiles` پروفایل ترجیحات و ویژگی‌های سفر هر کاربر را نگهداری می‌کند و 
از طریق رابطهٔ یک‌به‌یک `user` به مدل `Users` متصل است؛ شناسهٔ کاربر هم‌زمان کلید اصلی پروفایل
 محسوب می‌شود و با `related_name='profile'` می‌توان
 از طریق `user.profile` به آن دسترسی داشت. فیلدهای 
 این مدل اطلاعاتی مانند ملیت، زبان، بازهٔ سنی، بودجه، سبک سفر، نوع 
 همراهان، سطح فعالیت و محدودیت حرکتی، فصل مورد علاقه، ترجیح سفر
   مذهبی و گروه قومی را ذخیره می‌کنند و مقادیر پیش‌فرض آن‌ها امکان 
 ایجاد پروفایل اولیه برای کاربر را فراهم می‌سازد. این داده‌ها
   به‌عنوان ورودی موتور پیشنهاددهی استفاده می‌شوند تا 
 جاذبه‌های متناسب با علایق و شرایط هر کاربر انتخاب شوند.
   مقدار `managed=False` نیز نشان می‌دهد جدول `user_profiles` یک جدول
 موجود در پایگاه دادهٔ Legacy است و Django نباید مدیریت ساخت، تغییر یا حذف آن 
 را بر عهده بگیرد.
"""
