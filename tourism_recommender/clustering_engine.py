...
"""
این ماژول وظیفهٔ خوشه‌بندی داده‌ها با الگوریتم **Fuzzy C-Means (FCM)** و انتخاب خودکار بهترین تعداد 
خوشه را بر عهده دارد. تابع اصلی `fit_best_fcm` آرایهٔ ویژگی‌ها (`X`) را دریافت کرده و الگوریتم را برای تعداد 
خوشه‌های مختلف موجود در `CLUSTER_RANGE` اجرا می‌کند؛ اگر کتابخانهٔ `skfuzzy` نصب باشد از پیاده‌سازی
 آمادهٔ آن و در غیر این صورت از پیاده‌سازی داخلی `_fcm_fallback` استفاده می‌شود. در روش جایگزین، ابتدا 
ستون‌های داده با Min-Max نرمال‌سازی شده و ماتریس عضویت به‌صورت تصادفی مقداردهی می‌شود؛ سپس مراکز
 خوشه‌ها و میزان عضویت هر نمونه به‌صورت تکراری به‌روزرسانی می‌شوند تا تغییر مراکز از آستانهٔ `error` کمتر شود
 یا تعداد تکرارها به `max_iter` برسد. برای ارزیابی هر تعداد خوشه، سه معیار محاسبه می‌شود: ضریب `FPC` برای
 سنجش وضوح عضویت‌ها، شاخص `Xie-Beni` برای بررسی فشردگی و جدایی خوشه‌ها، و معیار `balance` برای 
اندازه‌گیری تعادل تعداد نمونه‌های خوشه‌ها. این معیارها نرمال‌سازی شده و با وزن‌های به‌ترتیب ۴۵، ۳۵ و ۲۰ درصد 
در یک امتیاز نهایی ترکیب می‌شوند؛ سپس گزینه‌ای که بیشترین امتیاز را دارد به‌عنوان بهترین مدل انتخاب شده و 
نتیجهٔ آن شامل تعداد خوشه‌ها، مراکز، ماتریس عضویت فازی، برچسب قطعی هر نمونه، معیارهای ارزیابی و تعداد
 اعضای هر خوشه در قالب `ClusteringResult` بازگردانده می‌شود.
"""
